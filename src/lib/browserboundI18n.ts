function browserboundI18nRuntime() {
	const STORAGE_KEY = 'browserbound.language';
	const CARD_ID = 'browserbound-language-settings';
	...
	const settingsCopy = {
		en: {
			...
			note: 'The original BrowserBound layout is preserved. This setting only adds a local translation layer.',
		...
		'zh-CN': {
			...
			label: '显示语言',
			note: 'BrowserBound 原始布局保持不变。此设置只添加本地翻译层。',
			applied: '语言偏好已保存。',
		},
		'zh-TW': {
			title: '語言',
			...
			label: '顯示語言',
			note: 'BrowserBound 原始版面保持不變。此設定只加入本機翻譯層。',
			applied: '語言偏好已儲存。',
		},
		es: {
			title: 'Idioma',
			...
			note: 'El diseño original de BrowserBound se conserva. Esta opción solo añade una capa local de traducción.',
		...
		fr: {
			...
			note: 'La mise en page BrowserBound d’origine est conservée. Ce réglage ajoute seulement une couche de traduction locale.',
		...
		de: {
			...
			note: 'Das ursprüngliche BrowserBound-Layout bleibt erhalten. Diese Einstellung ergänzt nur eine lokale Übersetzungsebene.',
		...
		ja: {
			...
			label: '表示言語',
			note: 'BrowserBound の元のレイアウトは維持されます。この設定はローカル翻訳レイヤーのみを追加します。',
			applied: '言語設定を保存しました。',
		},
		ko: {
			title: '언어',
			...
			label: '표시 언어',
			note: '원래 BrowserBound 레이아웃은 유지됩니다. 이 설정은 로컬 번역 레이어만 추가합니다.',
		...
		'pt-BR': {
			...
			note: 'O layout original do BrowserBound é preservado. Esta opção adiciona apenas uma camada local de tradução.',
		...
		ru: {
			...
			note: 'Исходная компоновка BrowserBound сохраняется. Эта настройка добавляет только локальный слой перевода.',
		...
		ar: {
			...
			label: 'لغة العرض',
			note: 'يتم الحفاظ على تخطيط BrowserBound الأصلي. يضيف هذا الخيار طبقة ترجمة محلية فقط.',
		...
		hi: {
			...
			label: 'प्रदर्शन भाषा',
			note: 'मूल BrowserBound लेआउट सुरक्षित रहता है। यह सेटिंग केवल स्थानीय अनुवाद परत जोड़ती है।',
	...
	function renderSettingsCard() {
		...
		main.insertAdjacentHTML(
			'afterbegin',
			`<section id="${CARD_ID}" class="container mx-auto flex-1 space-y-6 px-4 py-8 sm:px-6 lg:px-8">
				<div class="mx-auto max-w-3xl space-y-6">
					...
					<div class="rounded-2xl border border-border bg-card p-6 shadow-sm">
						<label class="mb-2 block text-sm font-medium text-foreground" for="browserbound-language-select">${text.label}</label>
						<select id="browserbound-language-select" class="h-11 w-full rounded-md border border-input bg-background px-3 text-sm text-foreground shadow-sm outline-none transition focus-visible:ring-2 focus-visible:ring-ring">
							${options}
						</select>
						<p class="mt-3 text-sm leading-6 text-muted-foreground">${text.note}</p>
						<p id="browserbound-language-status" class="mt-4 hidden rounded-md border border-border bg-muted px-3 py-2 text-sm text-muted-foreground">${text.applied}</p>
					</div>
				</div>
			</section>`,
		);
		document.getElementById('browserbound-language-select')?.addEventListener('change', (event) => {
			localStorage.setItem(STORAGE_KEY, event.target.value);
			renderSettingsCard();
			applyLanguage();
			const status = document.getElementById('browserbound-language-status');
			if (status) {
	...
	function patchNavigation() {
		for (const method of ['pushState', 'replaceState']) {
			...
			if (original.__browserboundLanguagePatched) continue;
			history[method] = function (...args) {
				const result = original.apply(this, args);
				window.setTimeout(schedule, 80);
				return result;
			};
			history[method].__browserboundLanguagePatched = true;
		}
		window.addEventListener('popstate', () => window.setTimeout(schedule, 80));
	}
...
export const browserboundI18nScript = `(${browserboundI18nRuntime.toString()})();`;
